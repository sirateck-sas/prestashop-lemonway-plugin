...
<div class="row">
    ...
    {if $oneclic_allowed == 1}
        <div class="Lemonway_payment_form">
            <img src="{$module_dir|escape:'html':'UTF-8'}views/img/paiement-mode.png" width="500px" class="img-responsive" alt="{l s='Pay with credit card' mod='lemonway'}" id="payment-lemonway-logo" />
            {l s='Pay with credit card' mod='lemonway'}
            <form id="placeOrderForm" action="{$link->getModuleLink('lemonway', 'redirect', array(), true)|escape:'htmlall':'UTF-8'}" method="POST" >
            ...
            {else}
                ...
                <div>
                    <label>
                        {l s='Actual card' mod='lemonway'} : <span>{$card_num|escape:'html':'UTF-8'}</span>
                    </label>
                </div>
                ...
                <div>
                    <label>
                        {l s='Expiration date' mod='lemonway'} : {$card_exp|escape:'html':'UTF-8'}
                    </label>
                </div>
                {/if}
    ...
    {else}
        <div class="Lemonway_payment_form Lemonway_payment_btn">
            <img src="{$module_dir|escape:'html':'UTF-8'}views/img/paiement-mode.png" width="500px" class="img-responsive" alt="{l s='Pay with credit card' mod='lemonway'}" id="payment-lemonway-logo" />
                {l s='Pay with credit card' mod='lemonway'}
            <form id="placeOrderForm" action="{$link->getModuleLink('lemonway', 'redirect', array(), true)|escape:'htmlall':'UTF-8'}" method="POST">
    {/if}
